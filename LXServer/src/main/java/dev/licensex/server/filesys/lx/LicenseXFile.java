...
import dev.licensex.server.exceptions.LXServerException;
import dev.licensex.server.utils.crypto.AES;
import dev.licensex.server.utils.jnic;
...
public class LicenseXFile {
    ...
    public LicenseXFile(String name_, String path_, List<String> content) throws LXServerException {
        this.name = (name_.endsWith(".lx") ? name_ : name_ + ".lx");
        this.path = (path_.endsWith(".lx") ? path_ : path_ + File.separator + name);
        ...
        if (!file.exists()) {
            ...
            throw new LXServerException("File already exists");
        }

        writeFile();
    }

    public LicenseXFile(String name_, String path_, List<String> content, boolean encryption_) throws LXServerException {
        this.name = (name_.endsWith(".lx") ? name_ : name_ + ".lx");
        this.path = (path_.endsWith(".lx") ? path_ : path_ + File.separator + name);
        this.content = content;
        this.encryption = encryption_;
